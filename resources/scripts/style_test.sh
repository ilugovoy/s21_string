...
echo -e "START STYLE TESTING src/*.c && *.h------>\t"
...
check_style() {
    ...
    if [[ ! -z "$diff_result" ]]; then
        echo -e "\033[0;31mФайл $file не соответствует стилю форматирования aka Google style\033[0m"
        ...
        if [[ "$answer" == "y" ]]; then
            ...
            mv "$file.tmp" "$file"
            echo -e "\033[0;32mФайл $file исправлен в соответствии со стилем форматирования Google\033[0m"
            
        elif [[ "$answer" == "n" ]]; then
            echo -e "\033[0;31mФайл $file оставлен в исходном состоянии. \nWARNING!!! ACHTUNG!!! ALARM!!! Если оставить файл в таком виде, хуевертер зафейлит проект!\033[0m"
            ...
            if [[ "$answer" == "n" ]]; then
                echo -e "\033[0;31mФайл $file оставлен в исходном состоянии. Вижу, ты не ищешь лёгких путей\033[0m"
                exit 1
            fi
...
files=$(find ../../ -maxdepth 2 -type f \( -name "*.c" -o -name "*.h" \))
